  ...
  async function handleDeleteList(_id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`/api/offplans?id=${_id}&agent=${agent}`)
          .then((res) => {
            if (res.data.success) {
              Swal.fire({
                title: "Deleted!",
      ...
      <div className="">
        <div className="flex items-center">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="">
                <h3 className="text-[20px] font-bold mb-2">
                  {title.slice(0, 23)} {title.length > 23 && "..."}
                </h3>
                <div className="flex gap-3 text-xs">
                  {/* bed */}
                  <div className="flex items-center gap-2">
                    <Image src={bed} alt="Bedroom svg" />
                    {bedroom.map((value, ind) => (
                      <span key={ind}>
                        <span>{value}</span>
                        {ind !== bedroom.length - 1 && <span>,</span>}
                      </span>
                    ))}
                  </div>
          ...
          <div className="w-[15%] text-center gap-3 flex items-center justify-center">
            <Link href={`/listing/Off-Plan/${list._id}`}>
              <Image
                src={live}
                alt="Live svg"
            ...
            <Link href={`listings/edit/${list._id}`}>
              {" "}
              <Image
                src={edit}
                alt="Live svg"
            ...
            <MdDelete
              size={20}
              className="text-red-500 hover:text-red-600 cursor-pointer hover:scale-125 transition-all"
