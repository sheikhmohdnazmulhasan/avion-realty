...
const PodcastCard = ({ podcast }) => {
      ...
      if (result.isConfirmed) {
        try {
          const serverResponse = await axios.delete(
            `https://www.avionrealty.ae/api/admin/podcast?id=${id}`
          );
          ...
          if (serverResponse.data.success) {
            ...
            mutate("https://www.avionrealty.ae/api/admin/podcast");
          }
        } catch (error) {
          console.log(error);
        }
  ...
  async function handleUpdatePodcast(event) {
    ...
    if (!urlRegex.test(videoUrl)) {
      ...
      axios.put(`https://www.avionrealty.ae/api/admin/podcast?id=${podcast._id}`, newData).then(res => {
