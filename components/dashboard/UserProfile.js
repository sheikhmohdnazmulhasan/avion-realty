...
const UserProfile = ({ user, mutate }) => {
  ...
  async function handleChangeDesignation() {
    ...
    if (designation.length > 20) {
      ...
      try {
        const serverResponse = await axios.put(
          `hhttps://avion-realty.vercel.app/api/users?email=${user?.email}`,
          dataWithDesignation
        );
  ...
  async function handleChangeProfilePicture(data) {
    ...
    try {
      ...
      if (imgBbResponse.data.success) {
        ...
        const serverResponse = await axios.put(`hhttps://avion-realty.vercel.app/api/users?email=${user?.email}`, dataForBackend);
