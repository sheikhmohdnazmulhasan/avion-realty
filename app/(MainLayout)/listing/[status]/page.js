...
const ListingDetail = ({ params }) => {
    ...
    const { data = [], isLoading, error } = useSWR(`hhttps://avion-realty.vercel.app/api/offplans?${params.status === 'Off-Plan' || params.status === 'Ready' || params.status === 'Rental' ? `status=${params.status}` : `area=${params.status}`}`, fetcher);
    ...
    const dataFilterByArea = (value) => {
        axios.get(`hhttps://avion-realty.vercel.app/api/offplans?area=${value}`).then(res => setListings(res.data)).catch(err => console.log(err))
    }

    const dataFilterByProperty = (value) => {
        axios.get(`hhttps://avion-realty.vercel.app/api/offplans?propertyType=${value}`).then(res => setListings(res.data)).catch(err => console.log(err))
    }
