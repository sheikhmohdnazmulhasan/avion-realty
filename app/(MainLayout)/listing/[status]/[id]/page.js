...
const ListingDetail = ({ params }) => {
    ...
    const { data = [] } = useSWR(`hhttps://avion-realty.vercel.app/api/offplans?id=${params.id}`, fetcher);
    const { data: agent = [] } = useSWR(`hhttps://avion-realty.vercel.app/api/users?email=${data.agent}`, fetcher);
