...
const AgentDetails = ({ params }) => {
    ...
    const { data: agent = [] } = useSWR(`https://www.avionrealty.ae/api/users?id=${params.id}`, fetcher);

    const { data: properties = [] } = useSWR(`https://www.avionrealty.ae/api/offplans?agent=${agent.email}`, fetcher);
