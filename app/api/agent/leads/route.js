  ...
  try {
    ...
    if (!result) {
      return NextResponse.json(
        {
          message: "Something Wrong for finding leads by provided list-id",
          success: false,
        },
        { status: 500 }
      );
    ...
    return NextResponse.json(
      { message: "Something Wrong in main tryc", success: false },
      { status: 500 }
    );
  }
}
...
export async function POST(required) {
  ...
  try {
    ...
    if (!list) {
      return NextResponse.json(
        {
          message: "Something Wrong for finding list by leadFor Id",
          success: false,
        },
        { status: 500 }
      );
      ...
      const updateLeadsCountInListing = await OffPlan.findByIdAndUpdate(
        data.leadFor,
        { leads: updateLeadsCount }
      );
      ...
      if (updateLeadsCountInListing) {
        ...
        if (!result) {
          return NextResponse.json(
            {
              message: "Something Wrong for creating new leads",
              success: false,
            },
            { status: 500 }
          );
