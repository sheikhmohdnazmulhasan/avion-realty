...
const EditBlog = ({ params }) => {
    const { data = [] } = useSWR(`hhttps://avion-realty.vercel.app/api/agent/blog?id=${params.id}`, fetcher);
    ...
    function handleCreateNewBlog(event) {
        ...
        if (!showName) {
            ...
            axios.put(`hhttps://avion-realty.vercel.app/api/agent/blog?id=${data._id}`, dataForBackend).then(res => {
        ...
        <div>
            ...
            <form className='mt-16 mr-8 space-y-8' onSubmit={handleCreateNewBlog}>
                <div className="">
                    ...
                    <input defaultValue={data.title} type="text" name="title" id='title' placeholder="Write Blog Title" className="bg-black text-xs p-2 rounded-md mt-1 w-full border border-dotted" required />
                </div>

                <div className="">
                    <label htmlFor="description">Description</label>
                    <textarea defaultValue={data.description} name="description" id="description" placeholder='Write Your Content Here' className='w-full p-2 h-64 bg-transparent border border-dotted rounded-md mt-1' required />
                </div>
                    ...
                    <div className=" mt-1 mb-10 ">
                        ...
                        {showName == 'prev' && (
                            <div className=" mx-auto flex items-center gap-x-6  rounded-md border border-dotted p-5">
                                <img className="size-[100px] h-[100px] w-full max-w-[150px] rounded-lg object-cover" src={data.blogImg} alt={showName?.name} />
