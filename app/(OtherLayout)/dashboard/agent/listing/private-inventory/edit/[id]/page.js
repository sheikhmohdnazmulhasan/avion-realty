...
const EditList = ({ params }) => {
    ...
    const { data = [], isLoading } = useSWR(`hhttps://avion-realty.vercel.app/api/inventory?id=${params.id}`, fetcher);
    ...
    const handleEditPlan = async (event) => {
        ...
        try {
            const serverResponse = await axios.put(`hhttps://avion-realty.vercel.app/api/inventory?id=${params.id}`, dataForBackend);
            ...
            if (serverResponse.data.success) {
                ...
                setTimeout(() => {
                    router.push('/dashboard/agent/listing/private-inventory')
                }, 1000)
            }
