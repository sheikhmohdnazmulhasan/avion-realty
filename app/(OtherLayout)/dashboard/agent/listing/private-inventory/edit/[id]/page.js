...
const EditList = ({ params }) => {
    ...
    const { data = [], isLoading } = useSWR(`http://localhost:3000/api/inventory?id=${params.id}`, fetcher);
    ...
    const handleEditPlan = async (event) => {
        ...
        try {
            const serverResponse = await axios.put(`http://localhost:3000/api/inventory?id=${params.id}`, dataForBackend);
