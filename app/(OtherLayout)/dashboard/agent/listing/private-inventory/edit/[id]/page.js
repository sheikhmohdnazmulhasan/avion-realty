...
const EditList = ({ params }) => {
    ...
    const { data = [], isLoading } = useSWR(`https://avion-realty.vercel.app/api/inventory?id=${params.id}`, fetcher);
    ...
    const handleEditPlan = async (event) => {
        ...
        try {
            const serverResponse = await axios.put(`https://avion-realty.vercel.app/api/inventory?id=${params.id}`, dataForBackend);
